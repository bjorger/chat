...
Both `fullStream` and `textStream` are supported. Use `fullStream` with multi-step agents — it preserves paragraph breaks between steps. Streams returned by TanStack AI's `chat()` are also auto-detected, with a paragraph break inserted between tool-loop turns. Any `AsyncIterable<string>` also works for custom streams.
