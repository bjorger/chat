...
This is for text responses. A structured-output run (`chat({ ..., outputSchema })`) streams its JSON as `TEXT_MESSAGE_CONTENT` deltas, so posting that stream would show raw JSON in the channel. Format the result yourself and post the string instead.
